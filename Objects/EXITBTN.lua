...
function EXITBTN:SetObjectVisibility(show)
	...
	if (CanExitVehicle() or UnitOnTaxi("player") or show or Neuron.buttonEditMode or Neuron.barEditMode or Neuron.bindingMode) and Neuron.ToggleAllFlag then --set alpha instead of :Show or :Hide, to avoid taint and to allow the button to appear in combat
		self:SetAlpha(1)
	else
		self:SetAlpha(0)
	end
